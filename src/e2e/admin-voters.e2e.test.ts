...
describe('E2E: Admin Voters Management', () => {
    ...
    describe('Voters List', () => {
        ...
        it('should have filter or search functionality', async () => {
            ...
            const searchInput = await ctx.page.$('input[type="search"], input[name="search"], input[placeholder*="search" i]');
            const pageContent = await ctx.page.content();
            // Either has search input or filter tabs/buttons
            const hasFiltering = searchInput !== null || 
    ...
    describe('Voter Approval', () => {
        ...
        it('should approve a pending voter', async () => {
            ...
            const approveBtn = await ctx.page.$('button[data-action="approve"], a[href*="approve"], form[action*="approve"] button');
            if (approveBtn) {
                await Promise.all([
    ...
    describe('Voter Rejection', () => {
        ...
        it('should reject a pending voter', async () => {
            ...
            const rejectBtn = await ctx.page.$('button[data-action="reject"], a[href*="reject"], form[action*="reject"] button');
            if (rejectBtn) {
                await Promise.all([
                    ctx.page.waitForNavigation({ waitUntil: 'networkidle0' }).catch(() => {}),
                    rejectBtn.click()
