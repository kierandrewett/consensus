    ...
    describe('Settings Update', () => {
        it('should update banner settings', async () => {
            ...
            if (bannerEnabledCheckbox) {
                const isChecked = await ctx.page.$eval('input[name="bannerEnabled"]', (el: HTMLInputElement) => el.checked);
                if (!isChecked) {
                    await ctx.page.click('input[name="bannerEnabled"]');
                }

                // Set banner message
                const bannerMessageInput = await ctx.page.$('input[name="bannerMessage"], textarea[name="bannerMessage"]');
                if (bannerMessageInput) {
                    await ctx.page.$eval('input[name="bannerMessage"], textarea[name="bannerMessage"]', (el: HTMLInputElement | HTMLTextAreaElement) => el.value = '');
                    await ctx.page.type('input[name="bannerMessage"], textarea[name="bannerMessage"]', 'E2E Test Banner Message');
                }
        ...
        it('should toggle maintenance mode', async () => {
            ...
            if (maintenanceCheckbox) {
                // Get current state
                const isChecked = await ctx.page.$eval('input[name="maintenanceMode"]', (el: HTMLInputElement) => el.checked);
                ...
                const newState = await ctx.page.$eval('input[name="maintenanceMode"]', (el: HTMLInputElement) => el.checked);
        ...
        it('should save auto-approval setting', async () => {
            ...
            if (autoApprovalCheckbox) {
                const isChecked = await ctx.page.$eval('input[name="autoApprovalEnabled"]', (el: HTMLInputElement) => el.checked);
                ...
                const newState = await ctx.page.$eval('input[name="autoApprovalEnabled"]', (el: HTMLInputElement) => el.checked);
                expect(newState).toBe(!isChecked);
