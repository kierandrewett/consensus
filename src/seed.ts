...
async function seedDatabase(): Promise<void> {
    ...
    const createVoter = async (data: VoterData): Promise<Voter> => {
        ...
        console.log(
            `  ✓ Voter: ${voter.name} (${voter.email}) - Status: ${voter.registrationStatus}, Password: ${data.password}`
        );
        return voter;
    };
    ...
    const createAdmin = async (username: string, password: string, fullName: string): Promise<Admin> => {
        const admin = new Admin(randomUUID(), username, await PasswordUtil.hash(password), fullName, new Date());
    ...
    const createElection = (data: ElectionData): Election => {
        ...
        console.log(
            `  ✓ Election: "${election.name}" (${election.electionType}) - ${startStr} to ${endStr} - Status: ${election.status}`
        );
            ...
            candidates: [
                {
                    ...
                    bio: "Community advocate with 10 years experience",
                },
                {
                    name: "John Davis",
                    party: "Conservative Alliance",
                    bio: "Former business owner focused on economic growth",
                },
            ...
            candidates: [
                {
                    ...
                    bio: "Second-year Law student, former class representative",
                },
    ...
    // Seed admins
    const adminsData = [{ username: "admin", password: "admin123", fullName: "System Administrator" }];
    ...
    const electionsByStatus = createdElections.reduce(
        (acc, e) => {
    ...
    const electionsByType = createdElections.reduce(
        (acc, e) => {
    ...
    console.log(`\nSummary:`);
    console.log(
        `  Elections:  ${createdElections.length} (${Object.entries(electionsByType)
            .map(([t, c]) => `${c} ${t}`)
            .join(", ")})`
    );
    ...
    console.log(
        `  Status:     ${Object.entries(electionsByStatus)
            .map(([s, c]) => `${c} ${s}`)
            .join(", ")}`
    );
}
