...
class MockBallotRepository implements IBallotRepository {
    ...
    findById(ballotID: string): Ballot | null {
        return this.ballots.find((b) => b.ballotID === ballotID) || null;
    }
    ...
    countByElectionId(electionID: string): number {
        return this.findByElectionId(electionID).length;
    }
...
class MockVoterEligibilityRepository implements IVoterEligibilityRepository {
    ...
    hasVoted(voterID: string, electionID: string): boolean {
        return this.voted.has(`${voterID}:${electionID}`);
    }
    markVoted(voterID: string, electionID: string): void {
        this.voted.add(`${voterID}:${electionID}`);
    }
}
...
class MockVoteConfirmationRepository implements IVoteConfirmationRepository {
    ...
    findByVoterId(voterID: string): VoteConfirmation[] {
        return this.confirmations.filter((c) => c.voterID === voterID);
    }
    findById(confirmationID: string): VoteConfirmation | null {
        return this.confirmations.find((c) => c.confirmationID === confirmationID) || null;
    }
}
