...
class MockVoterRepository implements IVoterRepository {
    ...
    save(voter: Voter): void { this.voters.set(voter.voterID, voter); }
    findById(voterID: string): Voter | null { return this.voters.get(voterID) || null; }
    ...
    update(voter: Voter): void { this.voters.set(voter.voterID, voter); }
    delete(voterID: string): void { this.voters.delete(voterID); }
    findAll(): Voter[] { return Array.from(this.voters.values()); }
    clear(): void { this.voters.clear(); }
}
...
describe('VoterService', () => {
    ...
    describe('FR1: Voter Registration', () => {
        ...
        it('should reject weak passwords', () => {
            expect(() => service.registerVoter({ ...validDTO, password: 'short' })).toThrow('Password must be at least 8 characters');
        });
    });
    ...
    describe('Voter Approval/Rejection', () => {
        ...
        beforeEach(() => {
            registeredVoter = service.registerVoter({ name: 'Jane', email: 'jane@example.com', password: 'password123' });
        });
