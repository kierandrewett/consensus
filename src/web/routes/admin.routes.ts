...
export async function adminRoutes(fastify: FastifyInstance, controllers: AdminControllers): Promise<void> {
    ...
    fastify.post<{ Body: { password: string; confirmPassword: string } }>(
    ...
    fastify.post<{ Params: { id: string }; Body: CandidateCreationRequest }>(
    ...
    fastify.post<{ Params: { electionId: string; candidateId: string } }>(
        "/admin/elections/:electionId/candidates/:candidateId/remove",
        async (request, reply) => {
